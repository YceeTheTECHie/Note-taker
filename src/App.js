...
class App extends Component {
	...
	render() {
		...
		const columns = [
			{ title: 'Notes', items: items.filter( item => item.status === 'To Do'), icon: <TodoIcon />},
			{ title: 'Done', items: items.filter( item => item.status === 'Done'), icon: <CheckIcon />},
			{ title: 'All Notes', items, icon: <ListIcon />},
		];
		return (
			<MuiThemeProvider>
				<div className="App">
					...
					<ConfirmDialog
						...
						message={'Are you sure you want to remove all Notes from the App?'}
						onCancel={this.handleDialogClose}
						onConfirm={this.clearTasks}
						open={this.state.dialogOpen}
					/>
					<AppBar
						title={<span style={{color: 'black', fontDisplay: 'Montserrat', fontWeight:'5em !important'}}>Note Taker</span>}
